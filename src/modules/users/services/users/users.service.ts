...
export class UsersService {
  ...
  constructor(
    ...
    @InjectRepository(StatuslUser) private statusRepository: Repository<StatuslUser>,
  ...
  async create(createUserDto: CreateUserDto) {
    ...
    var usr = await this.usersRepository.save({ ...createUserDto,genero: createUserDto.genero=='1' });
    //enviar contraseña cifrada
    await this.email.sendConfirmAccountAdmin(usr.nombre+' '+usr.apellido,usr.email,usr.nombre_usuario,password);
    return {
  ...
  async update(id:number,createUserDto: updateUserDto) {
    ...
    return (await this.usersRepository.update(id, { ...createUserDto,genero: createUserDto.genero=='1' })).affected
  }
