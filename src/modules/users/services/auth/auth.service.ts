...
export class AuthService {
   ...
   * @param contrasenia
   */
  async login_ecommerce(nombre_usuario: string, contrasenia: string): Promise<any> {
    const user = await this.usersRepository.findOne({email:nombre_usuario},{relations:['rol','estado']});
    if (user) {
      const isMatch = await bcrypt.compare(contrasenia, user.contrasenia);
      if (isMatch) {
  ...
  async register(createUserDto: RegisterUserDto) {
    ...
    await this.email.sendConfirmAccount(createUserDto.email,`${createUserDto.nombre} ${createUserDto.apellido}`,hash)
    return 'Ok';
  }
  ...
  async getRandomToken(email:string){
    return crypto.createHmac('sha256', CRYPTO_SECRET)
      .update(email)
      .digest('hex');
  }
  ...
  async confirmAccount(body:ConfirmAccountDto){
    const user:User = await this.usersRepository.findOne({where:{hash:body.key}});
    if(user){
      //actualizar 
      ...
      throw new HttpException('Solicitud de Confirmación Inválida', HttpStatus.BAD_REQUEST);
    }
  }
  ...
  async resetPassword(email:string){
    ...
    user.hash = hash;
    await user.save();
    await this.email.sendResetPassword(user.email,`${user.nombre} ${user.apellido}`,hash);
  }
  ...
  async newPassword(newPassword:NewPasswordDto){
    const user:User = await  this.usersRepository.findOne({where:{hash:newPassword.key}})
    if(!user){
      throw new HttpException('Enlace incorrecto',HttpStatus.NOT_FOUND)
    }
    user.hash = null;
    user.contrasenia =  await bcrypt.hash(newPassword.newPassword, this.saltOrRounds);
    await user.save();
  }
