...
export class InventoryService {
   ...
   */
  async reserveProduct(usuario: string, id: number, cantidad_reservada: number, cantidad: number, cantidad_anterior: number) {
  ...
  async moveProduct(moveStock: MoveStockDto, idUsuario: string) {
    ...
    try {
      ...
      let destino = await this.repository.findOne({
        where: { sucursal: moveStock.sdestino, producto: moveStock.producto_id },
      });
      if (origen) {
        ...
        const newCantOrigen = Number(origen.cantidad) - Number(moveStock.cantidad);
        const newCantDestino = (destino ? Number(destino.cantidad) : 0) + Number(moveStock.cantidad);
        if (destino) {
        ...
        await this.generarPdfMovement(log1.id, log2.id,moveStock.cantidad,nameFile)
        return true;
      } else {
        await queryRunner.rollbackTransaction();
        throw new HttpException(
  ...
  async getInventoryByProduct(id) {
    ...
    result.forEach(element => {
      info.push({
        ...
        precio_original: element.producto ? element.producto.precio_original : 0,
  ...
  async getInventoryByStore(id_sucursal: number) {
    ...
    return await this.repository.createQueryBuilder('inv')
   ...
   * @param cantidad_reservada
   */
  async cancelOrder(id_inventario: number, cantidad: number, cantidad_reservada: number) {
    return this.repository.update(id_inventario, { cantidad, cantidad_reservada })
  }
  ...
  async findToFill() {
    ...
    result.forEach(element => {
      array.push({
        ...
        proveedor: element.producto.proveedor ? element.producto.proveedor.nombre : '',
  ...
  async findToFillProduct(id: number) {
    let result = await this.repository.createQueryBuilder('inv')
      .leftJoinAndSelect('inv.producto', 'producto')
      .leftJoinAndSelect('inv.sucursal', 'sucursal')
      .leftJoinAndSelect('producto.proveedor', 'proveedor')
      .leftJoinAndSelect('producto.marca', 'marca')
  ...
  async findToQuote(id: number) {
    ...
    if(result.length > 0 ) {
      result[0].producto.imagenes = result[0].producto.imagenes.sort((a, b) => a.prioridad - b.prioridad)
      producto = {
   ...
   * @private
   */
  private async generarPdfMovement(idLog1: number, idLog2: number, cantidad: number,nameFile:string) {
